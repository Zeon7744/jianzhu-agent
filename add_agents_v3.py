# -*- coding: utf-8 -*-
"""Properly add 8 new agents to agents.py."""
import codecs
import os
import re
...
# Find the end of AGENT_PROFILES dict (the line with just '}')
# Look for the pattern: "name": "...", followed by "role" etc., ending with },\n}
# Find the last agent entry closing brace before COLLABORATION_MATRIX

# Strategy: find the position of COLLABORATION_MARKER, then work backward
# to find the end of the AGENT_PROFILES dict
marker = '\n\n# Agent间协作关系映射'
marker_idx = content.find(marker)
if marker_idx < 0:
    print("ERROR: Could not find COLLABORATION_MATRIX marker")
    exit(1)

# Go backwards from marker to find the closing } of AGENT_PROFILES
# Find the line that has just '}' before the marker
pos = marker_idx
# Go back to find the first } on its own line
while pos > 0 and content[pos] != '\n':
    pos -= 1
pos += 1  # at newline
# Now go back to find the closing brace
brace_pos = pos
while brace_pos > 0:
    if content[brace_pos] == '\n':
        next_line_start = brace_pos + 1
        # Check if this line is just '}'
        rest = content[next_line_start:].lstrip()
        if rest.startswith('}') and not rest.startswith('#'):
            # Found it - the dict ends here
            # But we need to insert BEFORE this closing brace
            insert_pos = next_line_start
            break
    brace_pos -= 1

if brace_pos <= 0:
    print("ERROR: Could not find closing brace of AGENT_PROFILES")
    exit(1)

print(f"Inserting at position {insert_pos}")
print(f"Context: {repr(content[insert_pos-50:insert_pos+50])}")

# Build new agent entries with proper indentation (4 spaces)
new_agents = '''    "\\u667a\\u5b89": {
        "id": "AG012",
        "name": "\\u667a\\u5b89",
        "role": "\\u5b89\\u5168\\u4e3b\\u7ba1",
...
decoded = codecs.decode(new_agents, 'unicode_escape')

# Insert BEFORE the closing } of AGENT_PROFILES
# We need to find the exact position
# Look for the pattern: the last "},\n" before the marker
last_comma_close = content.rfind('    },\n', 0, insert_pos)
if last_comma_close < 0:
    print("ERROR: Could not find insertion point")
    exit(1)

# Insert after the last "},\n"
insert_after = last_comma_close + len('    },\n')

content = content[:insert_after] + decoded + content[insert_after:]

# Now update COLLABORATION_MATRIX
col_idx = content.find('COLLABORATION_MATRIX = {')
if col_idx >= 0:
    # Find the closing of the dict
    depth = 1
    pos = col_idx + len('COLLABORATION_MATRIX = {')
    while pos < len(content) and depth > 0:
        if content[pos] == '{':
            ...
            depth -= 1
        pos += 1
    matrix_end = pos - 1  # position of the closing }
...
print(f"Total agents: {len(set(ids))}")
