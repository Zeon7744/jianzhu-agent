...
const initialStaff = [
  { id: 'EMP001', name: '张建国', dept: '技术部', position: '总工程师', cert: ['高级工程师', '注册结构工程师', '检测师'], phone: '138****1234', email: 'zhang@example.com', joinDate: '2018-03-15', status: 'active', avatar: '👨‍🔬' },
  { id: 'EMP002', name: '李明华', dept: '检测部', position: '检测主管', cert: ['检测师', '无损检测II级'], phone: '139****5678', email: 'li@example.com', joinDate: '2019-06-01', status: 'active', avatar: '👷' },
  { id: 'EMP003', name: '王小红', dept: '咨询部', position: '咨询工程师', cert: ['造价工程师', '咨询工程师'], phone: '137****9012', email: 'wang@example.com', joinDate: '2020-01-10', status: 'active', avatar: '👩‍💼' },
  { id: 'EMP004', name: '刘强', dept: '检测部', position: '检测员', cert: ['检测师'], phone: '136****3456', email: 'liu@example.com', joinDate: '2021-09-01', status: 'active', avatar: '🧑‍🔧' },
  { id: 'EMP005', name: '陈静', dept: '财务部', position: '财务主管', cert: ['会计师'], phone: '135****7890', email: 'chen@example.com', joinDate: '2019-11-20', status: 'active', avatar: '👩‍💻' },
  { id: 'EMP006', name: '周杰', dept: '检测部', position: '检测员', cert: ['检测师', '电工证'], phone: '134****2345', email: 'zhou@example.com', joinDate: '2022-03-15', status: 'leave', avatar: '👨‍🔧' },
  { id: 'EMP007', name: '吴芳', dept: '市场部', position: '客户经理', cert: [], phone: '133****6789', email: 'wu@example.com', joinDate: '2023-01-08', status: 'active', avatar: '👩' },
  { id: 'EMP008', name: '郑伟', dept: '技术部', position: '工程师', cert: ['检测师', '高级工程师'], phone: '132****0123', email: 'zheng@example.com', joinDate: '2020-07-20', status: 'active', avatar: '👨‍💼' },
]
...
export default function HR() {
  ...
  const stats = {
    ...
    certs: staff.reduce((s, st) => s + st.cert.length, 0),
    avgTenure: '3.2年'
  }
  ...
  const columns = [
    { title: '员工信息', width: 180, render: (_, r) => (
      <Space>
        <Avatar style={{ background: '#1890ff', fontSize: 18 }}>{r.avatar}</Avatar>
        <div>
    ...
    { title: '资质证书', width: 180, render: (_, r) => r.cert.length > 0 ? (
      <Tooltip title={r.cert.join(', ')}>
        <Space size={4} wrap>
          {r.cert.slice(0, 2).map((c, i) => <Tag key={i} color="blue" style={{ fontSize: 11 }}>{c}</Tag>)}
          {r.cert.length > 2 && <Tag style={{ fontSize: 11 }}>+{r.cert.length - 2}</Tag>}
        </Space>
      </Tooltip>
    ) : <Text type="secondary">无</Text> },
    { title: '入职时间', dataIndex: 'joinDate', width: 100 },
  ...
  return (
    <div className="page-container fade-in">
      ...
      <Row gutter={[16, 16]} style={{ marginBottom: 24 }}>
        {[
          { label: '在职人数', value: stats.total, icon: '👥', color: '#1890ff' },
          { label: '技术人员', value: stats.tech, icon: '🔧', color: '#52c41a' },
          { label: '证书总数', value: stats.certs, icon: '📜', color: '#faad14' },
          { label: '平均司龄', value: stats.avgTenure, icon: '📅', color: '#722ed1' },
        ].map((s, i) => (
      ...
      {/* 添加员工弹窗 */}
      <Modal title="添加员工" open={modalOpen} onOk={async () => {
        try {
          const values = await form.validateFields()
          const newStaff = { id: `EMP${String(staff.length + 1).padStart(3, '0')}`, ...values, status: 'active', cert: [], joinDate: dayjs().format('YYYY-MM-DD'), avatar: '👤' }
      ...
      {/* 档案弹窗 */}
      <Modal title={<Space><span>📁</span><span>员工档案</span></Space>} open={!!selectedStaff} onCancel={() => setSelectedStaff(null)} footer={null} width={600}>
        {selectedStaff && (
          <div>
            <Space style={{ marginBottom: 16 }}>
              <Avatar style={{ background: '#1890ff', width: 64, height: 64, fontSize: 32 }}>{selectedStaff.avatar}</Avatar>
              <div>
            ...
            <Row gutter={[16, 16]}>
              ...
              <Col span={12}><Text type="secondary">入职日期：</Text><br /><CalendarOutlined /> {selectedStaff.joinDate}</Col>
              <Col span={12}><Text type="secondary">资质证书：</Text><br /><CertificateOutlined /> {selectedStaff.cert.length > 0 ? selectedStaff.cert.join(', ') : '暂无'}</Col>
            </Row>
