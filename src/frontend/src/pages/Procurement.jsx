...
export default function Procurement() {
  const { user, procurement, fetchProcurement, suppliers, fetchSuppliers, materials, fetchMaterials } = useStore()
  ...
  const filteredSuppliers = suppliers.filter(s =>
    !searchText || s.name?.includes(searchText) || s.id?.includes(searchText)
  )
  ...
  const poColumns = [
    ...
    { title: '材料', dataIndex: 'material_id', width: 120 },
  ...
  return (
    <div className="page-container fade-in">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
        <div>
          <Title level={4} style={{ margin: 0 }}>采购管理</Title>
      ...
      <Card style={{ borderRadius: 12 }}>
        ...
        {activeTab === 'suppliers' ? (
          <Table dataSource={filteredSuppliers} columns={supColumns} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} scroll={{ x: 900 }} />
        ) : (
          <Table dataSource={procurement} columns={poColumns} rowKey="id" loading={loading} pagination={{ pageSize: 10 }} scroll={{ x: 1000 }} />
        )}
      </Card>
